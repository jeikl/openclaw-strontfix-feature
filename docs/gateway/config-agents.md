...
- `timeoutSeconds`: maximum seconds for a whole agent/chat run. Default: `172800` (48 hours). `0` disables the timeout. A finite value is raised to at least `tools.longTask.maxWaitMs` so a long exec cannot be cut off by a shorter run clock. Values already longer than `maxWaitMs` are unchanged.
