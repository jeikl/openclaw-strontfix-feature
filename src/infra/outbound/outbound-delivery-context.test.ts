...
describe("buildOutboundMessagePayload", () => {
  ...
  it("formats group-origin relay template with invoker fields", () => {
    ...
    expect(text).toContain("来源群id】原样复制：`cid123`");
    expect(text).toContain("调用人id】原样复制：`staff-1`");
    expect(text).toContain("【强制：message 工具 target 里的 ID 必须字节级原样复制");
    expect(text).toContain("禁止省略、截断、改写");
    expect(text).not.toContain("禁止去掉 `group:`");
  ...
  it("formats DM-origin template without group fields", () => {
    ...
    expect(text).toContain("【回复指引】");
    expect(text).toContain("调用人id】原样复制：`staff-9`");
    expect(text).toContain("【强制：message 工具 target 里的 ID 必须字节级原样复制");
    expect(text).toContain("禁止省略、截断、改写");
